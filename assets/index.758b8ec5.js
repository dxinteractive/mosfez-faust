...
`);var wn=nn?"destructors":"null",On=["throwBindingError","invoker","fn","runDestructors","retType","classParam"],In=[Vn,b,_e,xo,d[0],d[1]];tn&&(En+="var thisWired = classParam.toWireType("+wn+`, this);
`);for(var sn=0;sn<en-2;++sn)En+="var arg"+sn+"Wired = argType"+sn+".toWireType("+wn+", arg"+sn+"); // "+d[sn+2].name+`
`,On.push("argType"+sn),In.push(d[sn+2]);if(tn&&(bn="thisWired"+(bn.length>0?", ":"")+bn),En+=(pn?"var rv = ":"")+"invoker(fn"+(bn.length>0?", ":"")+bn+`);
