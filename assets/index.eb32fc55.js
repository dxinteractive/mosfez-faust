...
function createAudioCtx(audioCtxOrSampleRate) {
  ...
  if (audioCtxOrSampleRate === void 0) {
...
`;async function runInIframe$1(e){const{functionString:o,params:a,transferrables:i=[]}=e,c=document.createElement("iframe");c.style.display="none",document.body.append(c);const{contentWindow:h}=c;if(!h)throw new Error("contentWindow not available on iframe");return h.document.write(`<script>
    ...
    `,params:{channels:a,sampleRate:i,inputArrayBuffer:Z,length:g!=null?g:0,props:h},transferrables:$}),dn=await toAudioBuffer$1(rn,i);return toFloat32AudioArray$1(dn)}async function callback$1(){console.log("go");const e=[];for(let a=0;a<1e5;a++)e.push(.2);return console.log("result",await offlineRender$1({channels:2,sampleRate:48e3,input:[e,e],functionString:`exports.buildContext = async function buildContext(offlineCtx, source) {
