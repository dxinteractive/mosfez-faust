...
// ADDED: esm import of FaustModule
...
// Low-level API
faust.createWasmCDSPFactoryFromString = faust_module.cwrap(
  "createWasmCDSPFactoryFromString",
  "number",
  ["number", "number", "number", "number", "number", "number"]
);
faust.deleteAllWasmCDSPFactories = faust_module.cwrap(
  "deleteAllWasmCDSPFactories",
  null,
  []
);
faust.expandCDSPFromString = faust_module.cwrap(
  "expandCDSPFromString",
  "number",
  ["number", "number", "number", "number", "number", "number"]
);
faust.getCLibFaustVersion = faust_module.cwrap(
  "getCLibFaustVersion",
  "number",
  []
);
faust.getWasmCModule = faust_module.cwrap("getWasmCModule", "number", [
  "number",
]);
faust.getWasmCModuleSize = faust_module.cwrap("getWasmCModuleSize", "number", [
  "number",
]);
faust.getWasmCHelpers = faust_module.cwrap("getWasmCHelpers", "number", [
  "number",
]);
...
faust.cleanupAfterException = faust_module.cwrap(
  "cleanupAfterException",
  null,
  []
);
faust.getErrorAfterException = faust_module.cwrap(
...
faust.compileCode = function (factory_name, code, argv, internal_memory) {
  ...
  var argv_ptr_buffer = new Int32Array(
    faust_module.HEAP32.buffer,
    argv_ptr,
    argv_aux.length
  ); // Get a integer view on the newly allocated buffer.
  ...
  try {
    ...
    var module_code_ptr = faust.createWasmCDSPFactoryFromString(
      name_ptr,
      code_ptr,
      argv_aux.length,
      argv_ptr,
    ...
    if (module_code_ptr === 0) {
      ...
      argv_ptr_buffer = new Int32Array(
        faust_module.HEAP32.buffer,
        argv_ptr,
        argv_aux.length
      );
...
faust.createDSPFactoryAux = function (code, argv, internal_memory, callback) {
  ...
  var sha_key = Sha1.hash(
    code + (internal_memory ? "internal_memory" : "external_memory") + argv_str,
    true
  );
  ...
  var code_effect =
    "adapt(1,1) = _; adapt(2,2) = _,_; adapt(1,2) = _ <: _,_; adapt(2,1) = _,_ :> _; adaptor(F,G) = adapt(outputs(F),inputs(G)); dsp_code = environment{";
  ...
  if (res1) {
    ...
    if (res2) {
      ...
      faust.readDSPFactoryFromMachineAux(
        factory_name1,
        res1.code,
        res1.code_source,
        res1.helpers,
...
faust.expandDSP = function (code, argv) {
  ...
  try {
    var expand_dsp_ptr = faust.expandCDSPFromString(
      name_ptr,
      code_ptr,
      argv.length,
      argv_ptr,
...
faust.readDSPFactoryFromMachineAux = function (
  factory_name1,
  factory_code1,
  ...
  WebAssembly.compile(factory_code1)
    .then((module) => {
      // REMOVED: timing and console log
      //   var time2 = performance.now();
      ...
      // CHANGED: eval() is being used to write to globalThis
      // only to immediately pick it back up from global scope and assign it to variables
      // some bad habits here, polluting the global namespace for the sake of passing an evalled var from a to b :(
      // it wont run in a module either
      // should at least execute this in new Function()
      // and then you wouldnt even need to pass factory_name1 around as they could have the same name
      // but I wont refactor that as im trying to touch as little as possible
...
faust.createDSPInstance = function (factory, context, buffer_size, callback) {
  ...
  WebAssembly.instantiate(factory.module, importObject)
    .then((dsp_instance) => {
      ...
      var sp;
      try {
        sp = context.createScriptProcessor(
          buffer_size,
      ...
      // Setup buffer offset
      sp.audio_heap_inputs =
        sp.audio_heap_ptr_outputs + sp.numOut * sp.ptr_size;
      sp.audio_heap_outputs =
        sp.audio_heap_inputs + sp.numIn * buffer_size * sp.sample_size;
      ...
      sp.update_outputs = function () {
        if (
          ...
          for (var i = 0; i < sp.outputs_items.length; i++) {
            sp.output_handler(
              sp.outputs_items[i],
              sp.factory.getParamValue(
                sp.dsp,
                sp.pathTable[sp.outputs_items[i]]
              )
            );
          }
        }
        ...
        if (sp.numIn > 0) {
          ...
          for (i = 0; i < sp.numIn; i++) {
            sp.HEAP32[(sp.ins >> 2) + i] =
              sp.audio_heap_inputs + buffer_size * sp.sample_size * i;
          }

          // Prepare Ins buffer tables
          var dspInChans = sp.HEAP32.subarray(
            sp.ins >> 2,
          ...
          for (i = 0; i < sp.numIn; i++) {
            sp.dspInChannnels[i] = sp.HEAPF32.subarray(
              dspInChans[i] >> 2,
              (dspInChans[i] + buffer_size * sp.sample_size) >> 2
            );
          }
        }
        ...
        if (sp.numOut > 0) {
          ...
          for (i = 0; i < sp.numOut; i++) {
            sp.HEAP32[(sp.outs >> 2) + i] =
              sp.audio_heap_outputs + buffer_size * sp.sample_size * i;
          }

          // Prepare Out buffer tables
          var dspOutChans = sp.HEAP32.subarray(
            sp.outs >> 2,
          ...
          for (i = 0; i < sp.numOut; i++) {
            sp.dspOutChannnels[i] = sp.HEAPF32.subarray(
              dspOutChans[i] >> 2,
              (dspOutChans[i] + buffer_size * sp.sample_size) >> 2
            );
          }
        }
      ...
      sp.ctrlChange = function (channel, ctrl, value) {
        // REMOVED: the previous check here doesnt work in js as the author intended. we dont need it
        for (var i = 0; i < sp.fCtrlLabel[ctrl].length; i++) {
          ...
          sp.setParamValue(
            ...
            faust.remap(
              ...
              0,
              127,
              sp.fCtrlLabel[ctrl][i].min,
              sp.fCtrlLabel[ctrl][i].max
            )
          );
      ...
      sp.pitchWheel = function (channel, wheel) {
        for (var i = 0; i < sp.fPitchwheelLabel.length; i++) {
          ...
          sp.setParamValue(
            pw.path,
            faust.remap(wheel, 0, 16383, pw.min, pw.max)
          );
          if (sp.output_handler) {
...
faust.createDSPWorkletInstanceAux = function (factory, context, callback) {
  // Resume audio context each time...
  // ADDED: this throws errors if context hasnt started
  // it also returns a promise and the rest of the code in this function
  // should probably wait for this to resolve but for some reason it doesnt
  ...
  audio_node.getNumInputs = function () {
    return parseInt(factory.json_object.inputs);
  };
  audio_node.getNumOutputs = function () {
    return parseInt(factory.json_object.outputs);
  };
  ...
  audio_node.pitchWheel = function (channel, wheel) {
    for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
      ...
      this.setParamValue(
        pw.path,
        audio_node.remap(wheel, 0, 16383, pw.min, pw.max)
      );
      if (this.output_handler) {
...
/*faust.createMemory = function (factory, buffer_size, polyphony_aux) {
  ...
  var memory_size =
    pow2limit(
      (factory.effect_json_object
        ? parseInt(factory.effect_json_object.size)
        : 0) +
        parseInt(factory.json_object.size) * polyphony +
        (parseInt(factory.json_object.inputs) +
          parseInt(factory.json_object.outputs) * 2) *
...
// 'poly' DSP
// REMOVED: Fausts inbuilt version of polyphony is not required for this library

faust.createPolyDSPInstanceAux = function (
  factory,
  time1,
  mixer_instance,
  dsp_instance,
  ...
  try {
    sp = context.createScriptProcessor(
      buffer_size,
      parseInt(factory.json_object.inputs),
      parseInt(factory.json_object.outputs)
    );
  } catch (e) {
    faust.error_msg = "Error in createScriptProcessor: " + e;
    callback(null);
  ...
  sp.audio_heap_outputs =
    sp.audio_heap_inputs + sp.numIn * buffer_size * sp.sample_size;
  sp.audio_heap_mixing =
    sp.audio_heap_outputs + sp.numOut * buffer_size * sp.sample_size;

  // Setup DSP voices offset
  sp.dsp_start =
    sp.audio_heap_mixing + sp.numOut * buffer_size * sp.sample_size;
  ...
  sp.effect_start =
    sp.dsp_voices[polyphony - 1] + parseInt(factory.json_object.size);
  ...
  sp.getFreeVoice = function () {
    ...
    if (oldest_date_release != Number.MAX_VALUE) {
      if (faust.debug) {
        console.log(
          "Steal release voice : voice_date = %d cur_date = %d voice = %d",
          sp.dsp_voices_date[voice_release],
          sp.fDate,
          voice_release
        );
      }
      ...
      if (faust.debug) {
        console.log(
          "Steal playing voice : voice_date = %d cur_date = %d voice = %d",
          sp.dsp_voices_date[voice_playing],
          sp.fDate,
          voice_playing
        );
      }
  ...
  sp.compute = function (e) {
    ...
    for (i = 0; i < polyphony; i++) {
      if (sp.dsp_voices_state[i] != sp.kFreeVoice) {
        ...
        // Mix it in result
        sp.dsp_voices_level[i] = sp.mixer.mixVoice(
          buffer_size,
          sp.numOut,
          sp.mixing,
          sp.outs
  ...
  sp.initAux = function () {
    ...
    if (sp.numOut > 0) {
      ...
      for (i = 0; i < sp.numOut; i++) {
        sp.HEAP32[(sp.outs >> 2) + i] =
          sp.audio_heap_outputs + buffer_size * sp.sample_size * i;
        sp.HEAP32[(sp.mixing >> 2) + i] =
          sp.audio_heap_mixing + buffer_size * sp.sample_size * i;
      }
  ...
  sp.getJSON = function () {
    var res = "";
    res = res.concat('{"name":"');
    res = res.concat(factory.json_object.name);
    res = res.concat('",');
    res = res.concat('"version":"');
    res = res.concat(factory.json_object.version);
    res = res.concat('",');
    res = res.concat('"options":"');
    res = res.concat(factory.json_object.options);
    res = res.concat('",');
    res = res.concat('"inputs":"');
    res = res.concat(factory.json_object.inputs);
    res = res.concat('",');
    res = res.concat('"outputs":"');
    res = res.concat(factory.json_object.outputs);
    res = res.concat('",');
    res = res.concat('"meta":');
    res = res.concat(JSON.stringify(factory.json_object.meta));
    res = res.concat(",");
          ...
          if (factory.module_effect) {
            WebAssembly.instantiate(factory.module_effect, importObject)
              .then((effect_instance) =>
                faust.createPolyDSPInstanceAux(
                  factory,
                  time1,
...
faust.createPolyDSPWorkletInstanceAux = function (
  ...
  var audio_node = new AudioWorkletNode(
    context,
    factory.name + "_" + polyphony.toString() + "Poly",
    {
      numberOfInputs: parseInt(factory.json_object.inputs) > 0 ? 1 : 0,
      numberOfOutputs: parseInt(factory.json_object.outputs) > 0 ? 1 : 0,
      channelCount: Math.max(1, parseInt(factory.json_object.inputs)),
  ...
  audio_node.onprocessorerror = () => {
    console.log("An error from mydspPolyProcessor was detected.");
  };
  // Patch it with additional functions
  audio_node.handleMessage = function (event) {
    var msg = event.data;
...
faust.createPolyDSPWorkletInstance = function (
  ...
  if (factory.polyphony.indexOf(polyphony) === -1) {
    ...
    var mydspPolyProcessorString1 = mydspPolyProcessorString.replace(
      re1,
      factory.name + "_" + polyphony.toString()
    );
    var mydspPolyProcessorString2 = mydspPolyProcessorString1.replace(
      re2,
      polyphony
    );
    var mydspPolyProcessorString3 = mydspPolyProcessorString2.replace(
      re3,
      factory.getJSON()
    );
    var mydspPolyProcessorString4 = mydspPolyProcessorString3.replace(
      re4,
      factory.getBase64Code()
    );
    ...
    if (factory.name_effect) {
      var re5 = /CODE1/g;
      var re6 = /CODE2/g;
      var mydspPolyProcessorString5 = mydspPolyProcessorString4.replace(
        re5,
        factory.getJSONeffect()
      );
      var mydspPolyProcessorString6 = mydspPolyProcessorString5.replace(
        re6,
        factory.getBase64Codeeffect()
      );
      url = window.URL.createObjectURL(
        new Blob([mydspPolyProcessorString6], { type: "text/javascript" })
      );
    } else {
      var re5 = /CODE1/g;
      var re6 = /CODE2/g;
    ...
    context.audioWorklet
      ...
      .then(function () {
        ...
        // Create audio node
        faust.createPolyDSPWorkletInstanceAux(
          factory,
          context,
          polyphony,
          callback
      ...
      .catch(function (error) {
        console.log(error);
        console.log("Faust mydspPoly cannot be loaded or compiled");
        alert(error);
      });
  } else {
    // Create audio node
